  ...
  return (
    ...
    >
      <div className="group w-fit max-md:hidden">
        <nav className="flex h-8 w-fit items-center justify-evenly gap-3 lg:gap-6">
          {categories?.map((category) => (
            <div
              key={category.Title}
              className={cn(
                "flex items-center gap-[2px] text-[10px] capitalize text-gray-600 lg:text-sm",
                "hover:font-medium hover:text-sanskrutiRed hover:underline hover:underline-offset-4"
              )}
              onMouseEnter={() => {
        ...
        <div className="absolute left-1/2 top-8 hidden h-[25rem] w-full max-w-[60rem] -translate-x-1/2 border-x-2 border-b-2 border-gray-300 bg-white group-hover:block">
